...
N_canal_center = 50e3; % Center Canal effective pressure [Pa]
...
tau_c =@(x,y,u) (heaviside(sed_l - x).*(Cf*N_bedrock).*abs(u) ...
                 ...
                 (N_canal_side-N_infi)*(1-erf(gamma_side*abs(x-X_canal_sed_r)/W_max))+ ...
                 N_infi).*abs(u) ...
                 + heaviside(x - sed_r).*(Cf*N_bedrock).*abs(u)).*heaviside(905-y);
...
filename = fullfile("..","InputParameterFiles","Canal_"+string(N_canal_center/1000)+"kPa.mat");
save(filename)